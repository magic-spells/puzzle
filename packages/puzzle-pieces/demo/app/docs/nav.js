...
export const SECTIONS = [
	{
		...
		{ name: "calendar", title: "Calendar", path: "/components/calendar",
			description: "Inline month-grid calendar with days/months/years drill-down, full keyboard nav, min/max + disabledDates, locale-aware labels via Intl, caller-owned day-cell snippets, and an opt-in display-only range/preview shading overlay" },
		...
		{ name: "combobox", title: "Combobox", path: "/components/combobox",
			description: "Filterable autocomplete combobox with ranked filtering, groups, optional-controlled open, aria-activedescendant keyboard nav, and snippet-customizable option content" },
		...
		{ name: "date-picker", title: "Date Picker", path: "/components/date-picker",
			description: "Field-shaped trigger + popover composing Calendar: controlled ISO value, optional-controlled open, label/hint/error, locale-aware trigger label, auto-flip, forwarded day-cell snippets, and optional trigger↔panel morph" },
		{ name: "date-range-picker", title: "Date Range Picker", path: "/components/date-range-picker",
			description: "Start/end range selection in a field-shaped trigger + popover composing Calendar: controlled {start,end} ISO value, in-between range + hover-preview shading, optional-controlled open, one or two months, optional presets, and forwarded day-cell snippets" },
		...
		{ name: "markdown-editor", title: "Markdown Editor", path: "/components/markdown-editor",
			description: "WYSIWYG markdown editor on Tiptap/ProseMirror — Toolbar-driven headings, bold/italic/strike, inline code, lists, task lists, blockquote, code block, rules, links, and tables, with markdown typing shortcuts, emitting a controlled markdown string via @change; render stored markdown with the markdown piece" },
		{ name: "multi-select", title: "Multi-Select", path: "/components/multi-select",
			description: "Filterable tag / multi-value combobox with removable chips, ranked filtering, and snippet-customizable option content" },
		...
		{ name: "quantity-input", title: "Quantity Input", path: "/components/quantity-input",
			description: "E-commerce stepper: decrement/increment buttons around a number input, clamped to [min, max] with a configurable step and a real zero minimum; wraps the @magic-spells/quantity-input web component rather than porting it" },
		...
		{ name: "rich-text-editor", title: "Rich Text Editor", path: "/components/rich-text-editor",
			description: "Rich text editor on Tiptap/ProseMirror — a block-style dropdown (Normal text, Heading 1-4) plus Toolbar-driven bold/italic/underline/strike, alignment, lists, blockquote, code block, and links, emitting a controlled Shopify-style JSON document tree via @change; render stored docs with the rich-text piece" },
		...
		{ name: "select", title: "Select", path: "/components/select",
			description: "Listbox-pattern select as a compound family — options, group labels and dividers as real children, with typeahead, roving keyboard focus, disabled options, an optional-controlled panel and a replaceable trigger slot; wraps the @magic-spells/select-dropdown web component rather than porting it" },
		...
		{ name: "split-button", title: "Split Button", path: "/components/split-button",
			description: "Primary action button fused to a dropdown caret that opens a menu of secondary actions — the classic Save ▾ pattern, as a compound family over dropdown-menu with Button's variants and sizes" },
	...
	},
	{
		...
		{ name: "command", title: "Command", path: "/components/command",
			description: "Command palette with filtered, grouped, keyboard-driven quick actions and snippet-customizable option content" },
		{ name: "context-menu", title: "Context Menu", path: "/components/context-menu",
			description: "Right-click context menu — a compound family over the shared dropdown-panel base that opens at the pointer, clamped and flipped into the viewport, with dropdown-menu's rows re-exported and a touch long-press gesture" },
		...
		{ name: "dropdown-menu", title: "Dropdown Menu", path: "/components/dropdown-menu",
			description: "Menu-button dropdown — a compound family over the shared dropdown-panel base, with real role=\"menu\" semantics, typeahead, submenus, link rows, groups, labels, separators, shortcut hints, and disabled and danger items" },
		{ name: "dropdown-panel", title: "Dropdown Panel", path: "/components/dropdown-panel",
			description: "Shared base family for trigger↔panel overlays — hover/click disclosure with CSS-positioned panels, nested submenus, full-width mega panels, one-open-at-a-time arbitration, and the full arrow/Escape keyboard model; wraps the @magic-spells/dropdown-panel web component rather than porting it" },
		...
		{ name: "popover", title: "Popover", path: "/components/popover",
			description: "Click-toggled floating panel with arbitrary content — a compound family over the shared dropdown-panel base, with press-to-toggle latching, outside-press and Escape dismissal, focus return, and CSS-only end alignment" },
		{ name: "sheet", title: "Sheet", path: "/components/sheet",
			description: "One component that is a snap-point bottom sheet on phones and morphs — live, in place, while open — into a side drawer or centered dialog past the breakpoint, with spring-physics snaps, drag and flick gestures, a dismiss policy, entrance and exit effects, and an optional container-transform flight out of the trigger that opened it; wraps the @magic-spells/sheet web component rather than porting it" },
	...
	},
	{
		...
		{ name: "carousel", title: "Carousel", path: "/components/carousel",
			description: "Accessible scroll-snap carousel — arrows, dots, keyboard, autoplay; slides are slot children" },
		{ name: "code", title: "Code", path: "/components/code",
			description: "Code panel with syntax highlighting (highlight.js, GitHub light/dark palette, Puzzle template grammar), optional line-number gutter, copy button, and a collapsible View Code preview that animates open" },
		...
		{ name: "hamburger", title: "Hamburger", path: "/components/hamburger",
			description: "Menu button whose three bars fold into an X — converge, twist or slide, CSS-only two-phase motion on the individual translate/rotate properties, aria-expanded/aria-controls, optional-controlled open, an animate={false} instant swap, and bars in currentColor at sm/md/lg" },
		...
		{ name: "markdown", title: "Markdown", path: "/components/markdown",
		...
		{ name: "panel-stack", title: "Panel Stack", path: "/components/panel-stack",
			description: "Nested-panel navigation family — drill-down menus, settings flows and wizards, with the stack, the slide or stack transitions, inert handling, focus restoration, declarative push/pop triggers and Escape composition coming from the panel-stack web component" },
		{ name: "rich-text", title: "Rich Text", path: "/components/rich-text",
		...
		{ name: "tree", title: "Tree", path: "/components/tree",
			description: "Config-first WAI-ARIA tree view — nested nodes with expand/collapse, roving-tabindex keyboard nav, controlled selection, and a node snippet for caller-owned label rendering" },
	...
	},
	{
		...
		{ name: "split-panel", title: "Split Panel", path: "/components/split-panel",
			description: "Resizable pane group family — horizontal or vertical, any number of panes, with the drag and keyboard resizing, min/max constraints, snap points, nesting and localStorage persistence coming from the split-panel web component" },
		],
	},
];
