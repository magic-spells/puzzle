...
// back on screen, and its eventual real departure must still fire both.
...
describe('restored-then-departed hook order (D28 / D136 §3)', () => {
	it('a restored view fires no closing hook, and its real leave fires both', async () => {
		installFakeAnimate();
		const order = [];
		...
		// Cancelling the animation resolves the awaited `finished`, which used to
		// fall straight through to viewDidHide() on a view that is back on screen.
		...
		await tick();
		expect(order).toEqual(['willHide', 'willHide', 'didHide', 'destroyed']);
	...
	it('a hooks-only view restored and then removed still fires both hooks', async () => {
		...
		await view.destroyAnimated();
		await tick();
		expect(order).toEqual(['willHide', 'didHide', 'willHide', 'didHide', 'destroyed']);
	});
});
