...
func rejectComponentsAndSlots(nodes []Node, islandPos Position, file string) *ParseError {
	for _, n := range nodes {
		...
		case *Template:
			return errAt(file, node.Pos, "<Template> cannot appear inside an island element opened at %d:%d — it is caller-owned composition content", islandPos.Line, islandPos.Col)
		case *Portal:
