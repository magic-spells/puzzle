...
func TestReservedModuleScopeScriptBindings(t *testing.T) {
  ...
  <div class="static" data-count={ count }>no coercion</div>
	...
	tests := []struct {
		...
		{
			name:      "declared TEMPLATE_TAG with a scoped template",
			template:  templated,
			script:    "const TEMPLATE_TAG = 1;",
			wantIdent: "TEMPLATE_TAG",
		},
		...
		{name: "declared TEMPLATE_TAG without a scoped template", template: coercing, script: "const TEMPLATE_TAG = 1;"},
