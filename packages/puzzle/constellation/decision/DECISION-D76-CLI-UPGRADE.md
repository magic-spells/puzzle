...
- **The refresh is a separate process, not a goroutine.** `puzzle build` prints the notice as its last statement and exits in milliseconds; an in-process refresh dies with it, which is why the original background design never actually updated anything for `build` users. The CLI re-execs itself as `puzzle update-check` — a `Hidden: true` cobra subcommand that fetches with a 3 s budget, writes the cache or the failure stamp, prints nothing and exits 0 — started with `os.Executable()`, null standard streams, its own session (`Setsid`) or `DETACHED_PROCESS|CREATE_NEW_PROCESS_GROUP` on Windows, and `Start()` followed by a `Wait()` in a goroutine. The goroutine is the reaper: `Process.Release()` alone would leave the finished helper as a zombie for the life of a long-running parent (`puzzle dev` showed one `<defunct>` entry per server run under test), while a short-lived parent exits before the goroutine matters and init adopts and reaps the child. A spawn that fails is silent.
...
- **Cache writes are atomic** (temp file in the same directory, then rename; on Windows the rename retries briefly, because replacing a file another process holds open is a sharing violation there). Two builds started at once each spawn a helper, so concurrent writers are the normal case; a reader must never catch a half-written file and conclude the cache is corrupt. Duplicating the helper is cheap — it is one GET — so nothing coordinates or locks.
