...
- ~~Named slots~~ — shipped in v1.21 (§24, D53). ~~Scoped slots~~ shipped in v1.79 as **snippets** (§64, D166: a caller-declared `<Snippet fits="…" params>` body a component stamps per item through data attributes on its own markers). (Event modifiers, `{#unless}`, and multi-branch `{#case}` shipped in v1.7 — D36/D37/D38; the `{#switch}` name was rejected in favor of `{#case}`.)
...
- Virtual scrolling. A framework-owned virtual list stays out of scope, but §64 snippets removed the reason a *piece* could not own one: a windowed list component can now let the app decide what a row looks like.
...
- ~~Type-checking `.pzl` files~~ — shipped in v1.78 as `puzzle check` (§63, D165), an opt-in command that runs the app's own `tsc` over emitted virtual files. **Editor-level checking** (a language service or LSP) remains deferred, and deliberately so: every existing implementation of that shape is built on the TypeScript 6 JavaScript compiler API, which this project will not depend on. Revisit when the official TypeScript 7 tooling API ships.
