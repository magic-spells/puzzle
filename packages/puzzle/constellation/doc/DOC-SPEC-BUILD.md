...
- Cache: `<os.UserCacheDir()>/puzzle/update-check.json` (`checked_at` RFC3339 + `latest`, plus an optional `failed_at`; a file written without that field still loads), TTL **6 hours**. A cache inside the TTL answers on its own and no request is made at all. A **missing or stale** cache is refreshed **in the foreground, capped at 500 ms**, written, and the notice prints from that result **in the same run** — a release published since the last check is reported the first time the user runs `build`/`dev`, not on a later invocation. If that fetch exceeds the cap or fails, the fire-and-forget refresh (3s timeout) is started and the command answers immediately from whatever the stale cache held — but that refresh only pays off for a long-running `dev`: a short-lived `build` exits before it lands, so there the fresher answer shows up on a later run. The cap is therefore the hard bound on how long the passive path can delay any command; registry errors are never surfaced.
- **Failure backoff — 15 minutes.** A failed foreground fetch stamps `failed_at` in the cache file, leaving `latest`/`checked_at` exactly as they were, and while `now < failed_at + 15m` no fetch is attempted at all: the cached answer, if there is one, returns immediately. Without it a blackholing network — packets dropped rather than refused — charges *every* command the full cap indefinitely, because a failed fetch writes no `checked_at` and so nothing ever goes un-stale, and the async fallback never lands for a `build` that exits first. Any successful fetch (foreground or background) clears `failed_at`.
- On `dev` the notice is printed from the `OnReady` hook, which fires after the ready banner with the listener already accepting in its own goroutine — the bounded fetch delays neither the banner nor the first request.
