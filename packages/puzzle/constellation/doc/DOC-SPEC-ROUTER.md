...
- Path routing (off the pathname, via the HTML5 history API) by default; v1.6 adds opt-in hash mode via `routerMode: hashRouter()` (an import since D159) — see §15.
...
- **Programmatic history (v1.11, D42):** `router.go(n)` / `router.back()` / `router.forward()` in **all** modes — path/hash delegate to `history.go(n)` (the popstate path handles the rest); memory mode moves its internal stack index. Out-of-range `n` is a silent no-op (browser semantics).
- **Href encoding (v1.46, D79):** `router.url(path)` — path-shaped route in, mode-encoded href out: `base + path` (path mode), `'#' + base + path` (hash), unchanged (memory). Strings not starting with `/` pass through untouched (external URLs, `mailto:`, bare `#anchor` — the navigate-away escape hatch); a non-string argument throws. Query strings and `#anchor` suffixes ride along (pure prefixing). Templates reach it as the built-in `link` formatter (§6); it is the render-time inverse of the link interceptor and current-URL parsing.
...
**Config (`routerMode`) — an imported factory, not a string (D159).** Path routing is inline in the Router and is the zero-config default; the other two modes are opt-in imports, so an app that does not use them never bundles them:
...
- **Reading the current URL.** Path mode reads `location.pathname + location.search`; hash mode parses `location.hash` — `''`/`'#'` → `/`; `#/...` → that path (an in-fragment `?query` rides along); any other fragment (`#section2`) is **not a route fragment**.
...
**Listening is popstate-only, in both modes** (never `hashchange`): fragment navigations fire `popstate` in supported browsers — the same bet Vue Router 4 makes (its hash history is the HTML5 history with a `#` base). A `popstate` whose hash is a **non-route** fragment routes `/` on initial load but is **ignored entirely** on the pop — the rendered view is left alone, so an in-page anchor traversal never tears down the app. **Path mode reaches the same outcome by its own route (D41):** a pop whose path differs from the committed one in its **fragment alone** (`/docs` ⇄ `/docs#faq`) is settled in place, not navigated — the URL parts of the committed state move so `current.path`/`current.hash` stay honest, saved scroll is restored only if a position exists (a fresh fragment entry keeps the browser's own anchor landing), and nothing loads, refreshes, moves focus, or announces the route.
...
- Base-path support (hash-fragment base for sub-path hosting, and the path-mode equivalent — path mode assumes root deployment today). Deciding it properly means deciding it for both modes at once.
...
- **In a mounted view `this.route` mirrors the last COMMITTED navigation — fragments included.** An in-page anchor move (`/docs` ⇄ `/docs#faq`) is **not a navigation**: §15 routes it to the same-document fragment guard, which changes no route, no params and no chain, so it delivers no new snapshot and `this.route.hash` keeps the fragment the last navigation committed with. The snapshot commits with the tree (D146) and moves only when the tree does. The **live** fragment is `router.current.hash`, which the fragment pop updates in place — read that, not `this.route.hash`, when a view has to react to an anchor jump, and pair it with a `hashchange`/`popstate` listener, since an in-page move triggers no refresh and therefore no re-render.
...
- **Failure semantics are inherited from params, not widened:** a failed or superseded navigation changes neither the URL nor `router.current` (D19), and neither the params nor the snapshot of a reused ancestor. An ancestor's gated `data()` sees the destination through `this.route`/`this.params`, but that run commits only with the navigation (D146) — a rejection leaves the whole chain on the live route. On a **popstate** the browser moved the address bar before the router ran, so a pre-commit failure (blocked guard, no-op guard redirect, `lazy()` rejection, view-constructor throw, `data()` rejection) restores the committed URL with a `replaceState` — after any failed pop the address bar matches the DOM, and no history entry is added. The repair belongs to the navigation that failed: a failure superseded by a newer navigation restores nothing, leaving the winner's URL alone. A guard **redirect** continues the pop under the redirect target, and its own pre-commit failure repairs the same way — but only while the redirect chain still owns the location. That continuation tests its own chain token *alongside* the committed-state identity the sibling sites test; a bare token comparison would disable the repair outright, because re-entering the pipeline for the redirect bumps the router's token by construction.
...
- **Path mode:** URL is `/myapp/user/1`. The click interceptor intercepts only same-origin URLs **under the base** (stripped on push); same-origin links outside the base fall through to the browser — a real navigation away from the app. Loaded at a pathname outside the base: warn once, pathname passes through un-stripped (typically the catch-all).
...
**`router.replace(path)`.** Push's no-history-entry sibling: the identical match/load/cancellation/atomic-commit pipeline (§30 holds — a failed or superseded replace commits nothing), the same same-path no-op guard, and the same commit-window deferral. At commit: path mode `history.replaceState` (hash mode replaces the fragment entry) with the **current scroll-entry key kept** — it is the same history entry; memory mode overwrites `stack[index]` in place. **Replace never touches scroll by default** (a filter keystroke must not jump the page); a custom `scrollBehavior` (§14) still runs and may override. Static output's router stub throws for `replace` like every navigation method.
...
- **SSG (the only managed-tag path):** the shell injection replaces same-identity managed tags, removes ones whose field no longer resolves, and inserts the rest before `</head>` — escaped, deterministic string surgery (no HTML parser). The edit is confined to the **shell’s head region** (D151): the framework owns marker-bearing tags and the `<title>` element between `<head>` and `</head>`, and nothing else in the document. A `<title>` element or a `data-puzzle-head` attribute in RENDERED page markup is view output and is left byte-identical. A shell with no `</head>` degrades — the region ends after the first `</title>`, so the tags ride there; with neither anchor the inserts warn and are skipped, never a throw. Prerender results carry a resolved `head` beside the compatibility `title`. Because crawlers and unfurlers GET each URL fresh and never client-navigate, the tags baked into that page are always the copy they read.
...
- **Verdicts:** `undefined`/`true` allows. `false` blocks — stay put. A string path redirects: the **router** performs it through the same public seam as the denied navigation, **inheriting its verb** — a push redirect is a `push()` (§44) that mints one entry for the destination, so Back still returns to the page the user was on; a pop or navigation-#0 redirect is a `replace()` that takes over the entry the browser already sits on. Either way the denied URL never enters history (a push writes no entry until commit, §30), and the destination's own guards run normally through the standard pipeline. Guards may be async — the router awaits each one, and the cancellation token makes a superseded guarded navigation abandon silently. A guard that throws follows the data()-failure posture: logged, stay put.
- **Loop safety:** a guard redirect to the committed path is the §44 same-path no-op, whichever verb it inherits. At most ten guard redirects may run without an intervening commit; the next is treated as a cycle — `console.error`, stay put. A successful commit resets the counter.
...
- **`tabindex="-1"` is transient** — stamped before focusing, removed on the element's `blur`. A `<puzzle-view>` root is not natively focusable, but it must not become a permanent tab stop. **An author-set `tabindex` is never touched.** *(Amended, D139: the stamp also cuts both focus-ring channels for its lifetime — inline `outline: none !important` **and** `box-shadow: none !important` (Tailwind's `focus:ring-*` channel), prior inline values restored on the same blur. A keyboard-driven navigation makes `:focus-visible` match the freshly focused root and the UA draws its outline around the entire view; a programmatic-only target is not keyboard-operable, so the ring is noise. The author-set-`tabindex` branch gets no suppression either — the author chose that element's focus semantics, visuals included.)*
...
- **Skips:** memory mode is a full no-op (an embed shares the window with a host page — same reasoning as §14's scroll gate); navigation #0 does nothing, the browser owns first paint — the gate is *nothing committed yet* (`from == null`), so a guard redirect that re-enters as a `replace()` before anything has committed is still navigation #0 and moves no focus and makes no announcement, while a user push that supersedes a slow navigation #0 still focuses normally. Failed or superseded navigations never reach the commit point.
