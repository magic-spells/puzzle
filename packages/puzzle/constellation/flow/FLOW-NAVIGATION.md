...
4. **Run the guard chain** root → leaf, sequentially, before any view or layout is constructed.
   ...
   - a path string → redirect that inherits the denied navigation's verb — `push()` for a push, so the origin entry stays behind the destination; `replace()` for a pop or navigation zero — and the denied URL never enters history either way; ten redirects without an intervening commit trip the cycle cap
