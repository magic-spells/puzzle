...
object(s) — `loadOne`'s pk must match the requested id; create/update require
...
- **Caches, per Store, adapter-owned:** in-flight dedup by `recordKey`
  ...
  when the identity arrives by any path; confirmed `delete()` records absence,
  local `destroy()` does not; explicit `loadOne` bypasses it — force refresh);
  and the collection-complete type set (only a successful no-options
  collection load marks it; empty-array success counts; options-bearing loads
  stay partial).
...
- **Output modes:** prerender (hybrid + static) drives the same loop at build
  time — the API must be reachable from Node, §49's hook runs in the build
  context, and a non-404 fault failure fails the build naming the route.
  Static pages transfer read state in a second data island
