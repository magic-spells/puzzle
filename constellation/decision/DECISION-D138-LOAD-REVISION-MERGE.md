...
`loadAll`/`loadOne` merge through the same per-field revision gate `save()`
...
by any concurrent `loadAll`. Found as I11 of the 2026-07-27 pass-2 review;
...
- `loadAll(type)` snapshots `recordKey → recordMutationRevision(record)` for
...
Amends the §8 read path (D21/D137) with the §22/D125 merge gate.
