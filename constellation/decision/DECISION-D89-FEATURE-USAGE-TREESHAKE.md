...
- **head tags — deliberately coarse.** Route `meta` lives in `app/routes.js`, user JavaScript the compiler never parses (`.pzl` `<script>` bodies are likewise opaque). So this is a raw substring scan of `.js`/`.ts`/`.pzl` for `title`/`description`/`canonical`/`socialImage` — one probe per field `MANAGED_TAGS` derives a tag from. `title` is probed even though `<title>` itself belongs to the ungated title core: a route declaring only `meta: { title }` still resolves `og:title`/`twitter:title`, which the SSG injects unconditionally, so a false bit would strand those tags un-updatable for the life of the session — the false negative this scan must never produce. The original probe set omitted `title` and shipped exactly that bug. Probing it widens the over-inclusion noted under Consequences to nearly every app; correctness wins, and the gate is the marginal part. See the honest limitation there.
