...
- **`{#asset}` generic tag** — the contract (root-tag scan, innerHTML seed, parent styling) is SVG-specific; other asset types would each need their own semantics. Precise tags, general folder (`assets/`, not `svg/`). JSON needs nothing: `import x from './x.json'` in `<scripts>` already works via esbuild's built-in loader.
