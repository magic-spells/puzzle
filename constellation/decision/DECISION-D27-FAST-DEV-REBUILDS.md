...
- **Warm `tailwindcss --watch` child in `dev`.** When `puzzle.config.js` declares Tailwind, `puzzle dev` starts the resolved CLI with `--watch -i <input> -o <private file>` **once** at startup (an `os.CreateTemp` CSS file, never under `dist/`, so it is never served; removed on shutdown). `dist/styles.css` is re-composed (Tailwind layer + collected `<styles>`) whenever **either** side changes: (a) the child rewrites its output — an mtime poll of that one file (simplest reliable trigger; single-file fsnotify is fragile across atomic replaces) recomposes + reloads; (b) an esbuild rebuild changes the collected `<styles>` — recompose on that path. A single `.pzl` edit fires both, so reload broadcasts are **coalesced** within 100ms into one full-page reload. The child is killed on shutdown via the existing context: it runs in its own process group (`Setpgid` on unix via build-tagged `proc_unix.go`/`proc_other.go`) and is `SIGKILL`ed as a group (negative pid) so `node` dies with us; non-unix kills the process directly. **Gotcha (found in the manual proof):** the Tailwind v4 `--watch` CLI exits on stdin EOF, so a detached child inheriting `/dev/null` terminates right after its first build — the watcher holds a stdin pipe open for the child's lifetime to prevent this.
- **esbuild incremental context in `dev`.** `internal/build` exposes `NewWatchBuilder(root) → { Rebuild(); CSS(); Dispose() }` backed by `api.Context`, so rebuilds reuse esbuild's caches (only changed inputs re-read). `build.Build` is unchanged for production. The `.pzl` plugin's `<styles>` collector — now shared across incremental rebuilds via one persistent `Plugin` — was made reset-correct: `onLoad` set-or-**deletes** by `<styles>` presence (a file edited to drop its block loses its entry), and `CSS()` prunes entries whose file no longer exists (a deleted `.pzl`, whose `onLoad` never re-runs).
