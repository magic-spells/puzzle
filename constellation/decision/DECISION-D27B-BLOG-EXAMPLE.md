...
- **Plain `<style>` blocks, not Tailwind. — Superseded 2026-07-09; blog now uses Tailwind v4.** Originally blog shipped no `puzzle.config.js`, so the Tailwind runner was never invoked and the smoke build stayed Go + esbuild only (hermetic in CI, no node style pipeline), which also exercised the `<style>`-collection path that `examples/todos/` (Tailwind-first) does not. This was reversed for cross-example consistency: `examples/blog/` was converted to Tailwind v4 (`puzzle.config.js` + `app/styles/styles.css` with an `@theme` palette; all `.pzl` styled via utilities, zero `<style>`). Trade-off accepted: the pretest smoke build now runs the Tailwind CLI, so it is no longer node-free — CI installs `@tailwindcss/cli` via `npm ci`, and the `<style>`-collection path is still covered by unit tests + any component that emits `<style>`. See [[DOC-BLOG-EXAMPLE]].
